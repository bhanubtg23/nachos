...
AddrSpace::AddrSpace(OpenFile *executable)
{
    ...
    printf("Initialising address space: Code data: %d, Initialised Data: %d, Uninitalised Data: %d, User stack size: %d",noffH.code.size, noffH.initData.size, noffH.uninitData.size, UserStackSize);
    size=noffH.code.size + noffH.initData.size + noffH.uninitData.size + UserStackSize;
    printf("Total size: %d \n",size);
    numPages = divRoundUp(size, PageSize);
    //printf("numPages %d \n",numPages );
    size= numPages*PageSize;
    //printf(" new size: numpages * pagesize = %d \n",size);
    mLock->Acquire();
    int freepagecount=mem->GetFreePageCount();



//printf("freepagecount is - %d",freepagecount);

    if(numPages > freepagecount){
        valid=false;
        return;
    }
    printf("allocating pcb \n");	
	pcb = pcbManager->AllocatePCB();
    pcb->thread = currentThread;
    DEBUG('a', "Initializing address space, num pages %d, size %d\n", numPages, size);


    printf("PCB allocation completed \n");	
					
// first, set up the translation 
    pageTable = new TranslationEntry[numPages];
    for (i = 0; i < numPages; i++) {
        pageTable[i].virtualPage = i;	
        pageTable[i].physicalPage = mem->AllocatePage();
...
AddrSpace::~AddrSpace()
{
    for (int i = 0; i < numPages; i++) {
        mem->DeallocatePage(    pageTable[i].physicalPage );
    }
   delete pageTable;
    numPages=0;
}
